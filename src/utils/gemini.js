...
class GrantAI {
  constructor() {
    ...
    if (!apiKey) {
      console.warn('Google AI API key not found. Fallback responses will be used.');
      this.genAI = null;
    } else {
  ...
  async analyzeGrantCompatibility(businessProfile, grantDetails) {
    if (!this.genAI) {
...
`;
  ...
  async generateGrantRecommendations(businessProfile, availableGrants) {
    if (!this.genAI) {
...
`;
  ...
  async generateApplicationGuidance(grantDetails, businessProfile) {
    if (!this.genAI) {
...
`;
  ...
  async generateApplicationDraft(grantDetails, businessProfile, section) {
    if (!this.genAI) {
...
`;
  ...
  async analyzeApplicationQuality(applicationData, grantRequirements) {
    if (!this.genAI) {
...
`;
  ...
  getFallbackRecommendations(grants) {
    return {
